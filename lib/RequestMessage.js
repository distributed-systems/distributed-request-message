!function() {
	...
	module.exports = new Class({
		...
		, send: function(messageHandler, callback) {
			var promise, reject, resolve, err;
			...
			if (!this._isSent) {
				if (type.object(messageHandler) && type.function(messageHandler.emitMessage)) {
                    ...
                    err = new Error('Cannot send the message, there was no messageHandler passed to it (via the classes constructor options or the send method)!');
                    if (callback) callback(err);
                    else return Promise.reject(err)
                }
			}
			else {
                err = new Error('Cannot send message, the message was already sent!');
                if (callback) callback(err);
                else return Promise.reject(err)
            }
		}
         ...
         * @param <string> error code, machine readable
         */
        , sendError: function(status, description, code) {
            var content = {};

            if (!description) throw new Error('The description for the sendError method is not optional!');

            content.description = description;
            content.code = code || status;

            this.sendResponse(status, content);
        }
		...
		, _prepareResponse: function(responseMessage, args) {
            ...
            // validate message
            if (!responseMessage instanceof ResponseMessage) throw new Error('Cannot prepare a message that doesn\'t inherit from the ResponseMessage type!');
            ...
            if (!type.string(status)) throw new Error('Got no statuscode for the response message!');

            // a status is not optional
